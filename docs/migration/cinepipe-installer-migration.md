...
**Status**: handoff document, not an MLAppInstaller implementation plan — nothing here executes automatically. Written 2026-08-16 against MLAppInstaller's current state (`mlai-core`, `mlai-cli`, `mlai-credentials`, `mlai-gui`, model catalog).
...
**GUI wizard: `add_project`/project-binding was dropped, not ported.** `mlai-gui` intentionally does not include cinepipe's UE5 project-binding feature (`BindsToProjectType`, the "Add Project" mode, the native file-picker dialog) — it isn't generalized in MLAppInstaller's manifest schema yet. If CinePipe needs this in production, it has two paths: keep running the existing wizard fork for that one feature until project-binding is generalized, or drive its own generalization (as a manifest field + resolver, following the same pattern the per-platform setup retrofit already used).
...
`mlai-package` (native MSI/dmg/deb generation wrapping `cargo-packager`, signing-as-reference, GitHub Releases deploy adapter) and `mlai init` (the guided distribution-authoring wizard) are **designed but not built** — see `docs/superpowers/specs/2026-08-15-distribution-packaging-framework-design.md`. `UNIFIED-INSTALLER-DESIGN.md`'s own "Reliability prerequisites" (a real CI matrix; end-to-end verification on all target platforms, not just the build platform) are **already satisfied here** — this project's CI runs and passes on `ubuntu-latest`/`macos-latest`/`windows-latest` today, which is more cross-platform proof than `feat/unified-rust-installer` had on its own.
...
4. **Now, if the GUI is wanted before project-binding is generalized**: either keep the existing wizard fork running for that one feature, or contribute a project-binding generalization upstream.
5. **Later, once `mlai-package` exists**: author a distribution profile (signing-identity references for the existing Windows/macOS certs, GitHub Releases as the deploy target) to replace the packaging half of `UNIFIED-INSTALLER-DESIGN.md` — licensing/activation-gating stays exactly as separately designed there.
6. **Retire** `feat/unified-rust-installer` once its provably-migrated pieces are confirmed working from MLAppInstaller directly, rather than maintaining both.
