...
MLAppInstaller's founding scope names "configurable local-vs-cloud model backend selection at install time" as core, but nothing built so far treats *which ML models/tools ship in a given distribution* as a first-class packaging concern — it's been implicit, left to each component's own setup script. Separately, this project was asked to be a reusable foundation, not a shipped product: no adopting project needs MLAppInstaller to have its own release pipeline — they need a **framework that makes it easy for them to build their own distributions**: which components/models are included, for which target machines, packaged into which native installer format, signed how, published where.

A partner project's own prior art (`model-catalog.json`) already solved the "two components silently disagree about which model fits a given machine" problem — but did so assuming a single team could author one canonical file. That project today is a loose collection of sub-projects with no such central authority, and the fix needs to account for that.
...
- `mlai init`: a guided CLI wizard for the *adopter* (an engineer at an adopting project configuring their distribution) — not an end-customer-facing GUI installer.
...
- **Real cross-platform hardware auto-detection** (parsing `nvidia-smi`, Metal/`system_profiler`, WMI). Today, every component in the partner project's own stack already does its own hardware detection inside its own setup script; this framework's catalog *resolver* accepts a `HardwareProfile` as input without caring how it was produced. A `mlai hardware detect` auto-detection utility is a valuable, real, separately-scoped follow-up — not required for the catalog/merge/resolve mechanism to work or be tested.

## Architecture

### 1. Model catalog (`mlai-core`)

**The ownership problem, and why merge-with-conflict-detection instead of a single file or silent coalescing:** the partner project's existing catalog assumes one author. In reality, a second sub-project might independently determine a better model for a hardware tier with no central team to update a shared file. The fix is **catalog fragments with explicit per-purpose ownership**:

```toml
# studio-a's model-catalog.toml fragment
...
- Same purpose, **different** tier data from a **non-owning** fragment → **hard error**, loud and actionable, naming both fragments and the conflicting values. This is the exact failure mode that caused the partner project's original bug (two products independently inventing different tier tables for the same purpose) — the framework refuses to let it happen silently rather than trying to reconcile it automatically.
...
`effective_vram_gb` generalizes two concepts the partner project's own catalog comments already named but hadn't formalized: **Apple Silicon unified-memory derating** (the exact axis that caused their original bug) and **reservation** (subtracting headroom for a co-resident heavy GPU consumer like a real-time 3D engine — their catalog's comment explicitly says this was "not built yet"; this design completes it as a resolver parameter, `reserve_vram_gb`, rather than each product hand-rolling its own math).
...
platform = "macos"
format = "dmg"
sign.identity_ref = "keychain:Developer ID Application: Example Studio, Inc."
...
2. **Model catalog generalizes a prior-art project's mechanism (ownership + shared resolution), not its data.** Each adopting project authors its own catalog fragments for its own domain; cross-project data sharing between different adopting projects isn't meaningful since their model needs differ.
...
## Relationship to adopter migration mapping

Per explicit sequencing: this framework is designed and built first. Once it exists, a separate mapping document — not part of this spec — shows concretely what in an adopting project's existing installers/catalogs maps onto it, and what each project would delete. See `docs/migration/adopter-migration-guide.md` and `docs/migration/ta-migration.md`.
