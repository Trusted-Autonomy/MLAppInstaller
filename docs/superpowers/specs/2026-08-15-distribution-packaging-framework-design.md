...
**Signing is reference-only, confirmed as the ecosystem norm, not something to invent:** `cargo-packager` itself doesn't sign or notarize — it shells out to `codesign`/`signtool`/`notarytool` given an identity reference (a keychain identity name, a certificate-store thumbprint, a PFX path + a *named* environment variable for the password). `mlai-package`'s distribution profile only ever carries these references. Actual secret material lives in the adopter's own CI secret store — exactly the same principle already established for credentials in `docs/CONSTITUTION.md` §2.1, applied to a second domain.
